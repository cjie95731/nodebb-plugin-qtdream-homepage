(function(module) {
	...
	{
		app = params.app;
		helpers.setupPageRoute( params.router, '/custom', params.middleware, [ ],
			function ( req, res )
			{
				res.render( "qtdream",
					{
				...
				function ( post, next )
				{
					...
					if ( post && post.content )
					{
						...
						if ( i >= 0 )
						{
                            ...
                            if(txt.search("https://img")===0)
                            {
                                txt = (txt.match(/\w+$/)=='jpg') ? (txt.replace(/\w+$/,"md.jpg")) : txt.replace(/\w+$/,"md.png");
                            }
                            cover = txt;
						}
					}
